...
export default function FamilyTableSection({ families = [], loading, onDelete }) {
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-green-800">
                      {families.reduce((total, family) => total + (family.members?.length || 0), 0)}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-purple-800">
                      {families.length > 0 
                        ? Math.round((families.reduce((total, family) => total + (family.members?.length || 0), 0) / families.length) * 10) / 10 
                        : 0}
                    </p>
                  </div>
                  ...
                  {families.map((family, index) => (
                    <tr key={family.id} className={`hover:bg-blue-50 transition-all duration-200 ${index % 2 === 0 ? 'bg-blue-25' : 'bg-white'}`}>
                      <td className="px-6 py-4">
                        <span className="text-blue-600 font-mono font-semibold">{family.familyNumber || 'N/A'}</span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="font-semibold text-gray-900">{family.headOfFamily || 'N/A'}</div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-gray-700">{family.household?.incomeBracket || 'N/A'}</td>
                      <td className="px-6 py-4 text-gray-700">{family.household?.houseType || 'N/A'}</td>
                      <td className="px-6 py-4">
