...
export default function ViewBlotterModal({ isOpen, onClose, blotterId }) {
    ...
    const handleUpdateStatus = async () => {
        ...
        try {
            ...
            dispatch(get_blotters_thunk()); // Refresh the list
            onClose();
        } catch (error) {
        ...
        <Modal isOpen={isOpen} onClose={onClose} maxWidth="4xl">
            <div className="bg-white rounded-lg max-h-[90vh] overflow-y-auto">
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-blue-50 p-4 rounded-lg">
                            ...
                            <p className="text-lg font-bold text-blue-600 mt-1">
                                {blotter.id ? `BLT-${String(blotter.id).padStart(6, '0')}` : 'N/A'}
                            </p>
                        </div>
                        ...
                        <div className="space-y-2">
                            {blotter.complainant_resident && (
                                <div>
                                    ...
                                    <p className="font-medium">{blotter.complainant_resident}</p>
                                </div>
                            )}
                            {blotter.complainant_not_resident && (
                                <div>
                                    <label className="text-sm text-gray-600">Non-Resident Name:</label>
                                    <p className="font-medium">{blotter.complainant_not_resident}</p>
                                </div>
                            )}
                        ...
                        <div className="space-y-2">
                            {blotter.respondent && (
                                <div>
                                    ...
                                    <p className="font-medium">{blotter.respondent}</p>
                                </div>
                            )}
                            {blotter.respondentResident && (
                                <div>
                                    <label className="text-sm text-gray-600">Registered Resident:</label>
                                    <p className="font-medium">
                                        {`${blotter.respondentResident.firstName} ${blotter.respondentResident.middleName || ''} ${blotter.respondentResident.lastName}`}
                                    </p>
                                </div>
                            )}
                        </div>
                    ...
                    {/* Persons Involved */}
                    {(blotter.person_involved_resident || blotter.person_involved_not_resident) && (
                        <div className="bg-gray-50 p-4 rounded-lg">
                            <h3 className="font-semibold text-gray-800 mb-3">Persons Involved</h3>
                            <div className="space-y-2">
                                {blotter.person_involved_resident && (
                                    <div>
                                        <label className="text-sm text-gray-600">Resident:</label>
                                        <p className="font-medium">{blotter.person_involved_resident}</p>
                                    </div>
                                )}
                                {blotter.person_involved_not_resident && (
                                    <div>
                                        <label className="text-sm text-gray-600">Non-Resident:</label>
                                        <p className="font-medium">{blotter.person_involved_not_resident}</p>
                                    </div>
                                )}
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <p className="font-medium">{blotter.location_of_incident || 'N/A'}</p>
                            </div>
                            <div>
                                <label className="text-sm text-gray-600 flex items-center gap-1">
                                ...
                                <p className="font-medium">{blotter.date_of_incident || 'N/A'}</p>
                            </div>
                            <div>
                                <label className="text-sm text-gray-600 flex items-center gap-1">
                                    <Calendar className="w-4 h-4" />
                                ...
                                <p className="font-medium">{blotter.date_reported || 'N/A'}</p>
                            </div>
                        </div>
                        <div className="mt-3">
