...
import { User, MapPin, Home, Users, Phone, Calendar, AlertCircle } from 'lucide-react';
import moment from 'moment';
import Modal from '@/app/_components/modal';

export default function ViewDetailSection({ isOpen, onClose, official }) {
    if (!official || !official.resident) return null;
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} width="max-w-5xl">
            <div className="flex flex-col max-h-[80vh]">
                {/* Header */}
                <div className="bg-gradient-to-r from-red-600 to-red-700 text-white p-6 -m-6 mb-6 rounded-t-xl">
                    <div>
                        <h2 className="text-2xl font-bold">Former Official Details</h2>
                ...
                <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6 flex items-start gap-3">
                    ...
                    <div>
                        <h4 className="font-semibold text-red-900 mb-1">Term Has Ended</h4>
                        <p className="text-sm text-red-700">
                            This person is no longer serving as <strong>{official.position}</strong>. 
                            Their term ended on {official.termTo ? moment(official.termTo).format("MMMM DD, YYYY") : "N/A"}.
                        </p>
                    </div>
                </div>
                ...
                <div className="overflow-y-auto flex-1 pr-2">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-lg font-semibold text-red-600">{resident.status || "NOT ACTIVE"}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {/* Former Official Information */}
                        <div className="bg-white border border-red-200 rounded-lg p-5">
                            <SectionTitle icon={Calendar} title="Former Official Information" />
                            <InfoItem label="Position Held" value={official.position} />
                            <InfoItem label="Term Start Date" value={official.termFrom ? moment(official.termFrom).format("MMMM DD, YYYY") : "N/A"} />
                            <InfoItem label="Term End Date" value={official.termTo ? moment(official.termTo).format("MMMM DD, YYYY") : "N/A"} />
                            <InfoItem label="Date Archived" value={official.dateDeleted ? moment(official.dateDeleted).format("MMMM DD, YYYY") : "N/A"} />
                        </div>
